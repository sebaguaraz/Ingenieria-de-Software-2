...
testUtils.createTestButton("Test Eliminar Sample Dinámico", async (button) => {

    await okLogin();
    const token = localStorage.getItem('test_token');

    const listExist = await getMySamples()
    if (listExist.length === 0) {
        testUtils.log({ message: "Lista Vacia, se debe subir un sample primero" })
        return
    }

    const firstId = listExist[0].id

    const response = await fetch(`/api/samples/${firstId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
    });
...
testUtils.createTestButton("Test Subir Sample - Error por Datos Faltantes", async (button) => {
